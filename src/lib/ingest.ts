  ...
  return [
    ...
    isExperience
      ? [
          ...
          "",
          '## 0. Manually Flagged Experiences ([EXP] markers)',
          '- The source may contain [EXP] ... note blocks. These were manually',
          '  flagged by the user as high-priority. Treat them as the most important',
          '  signals — every [EXP] marker should result in a corresponding page.',
          '',
          "## 1. Bugs & Defects",
          ...
          "",
          "## 7. Status Detection",
          "- For each bug: determine if it was resolved in this session.",
          '  * `unresolved` — the problem was discovered but NOT fixed.',
          '  * `resolved` — the problem was diagnosed AND fixed.',
          '  * `cannot-reproduce` — environment-specific, not reproducible.',
          '  * `wont-fix` — intentionally not fixed (cost, risk, not-a-bug).',
          "- For each decision: determine its state.",
          '  * `proposed` — options were discussed but no choice was made yet.',
          '  * `accepted` — a choice was made and is being acted on.',
          '  * `deprecated` — the decision was once used but no longer applies.',
  ...
  return [
    ...
    isExperience
      ...
      : [
          ...
          "5. An updated wiki/log.md — append new entries using this format:",
          "   ```",
          "   ## YYYY-MM-DD",
          "   ",
          ...
          "   ```",
          "   Prefix rules: Fixed=resolved bug, New=new page created, Linked=bug→pattern, Updated=content changed, Deprecated=decision obsolete.",
          "   Always use [[wikilink]] syntax for page references. Only list pages generated or changed THIS session.",
    ...
    isExperience
      ? [
          ...
          "  • status   — (bug pages) unresolved | resolved | cannot-reproduce | wont-fix. Default: unresolved.",
          '  • resolution — (bug pages with status=resolved) one-line fix summary in quotes.',
          "  • status   — (decision pages) proposed | accepted | deprecated | superseded. Default: accepted.",
          '  • superseded_by — (decision pages with status=superseded) slug of replacement decision.',
        ].join("\n")
      : "",
    "",
...
function buildAggregateRepairPrompt(
  ...
  return [
    ...
    "- For wiki/log.md: output only the new log entry to append. Format:",
    "  ```",
    "  ## YYYY-MM-DD",
    "  ",
    ...
    "  - Fixed [[slug]] — <one-line>",
    "  - New [[slug]] — <one-line>",
    "  - Linked [[slug]] → pattern — <one-line>",
    "  ```",
    "  Use the correct prefix (Fixed/New/Linked/Updated/Deprecated) and include [[wikilink]] references.",
